IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210326064411_personNewAuditApiPermission')
...
--'/api/audit-person-profile/{peopleIndex}/{historyType}' starts
IF NOT EXISTS(select ApplicationObjectID from ApplicationObject where [Name] = '/api/audit-person-profile/{peopleIndex}/{historyType}')
   BEGIN
     INSERT INTO ApplicationObject VALUES(3,'/api/audit-person-profile/{peopleIndex}/{historyType}','GetAuditPersonProfileDetails',1, 0,GETDATE(),1)
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/audit-person-profile/{peopleIndex}/{historyType}')
   END
ELSE
   BEGIN
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/audit-person-profile/{peopleIndex}/{historyType}')
   END
insert into info.Permission values(@appObjId,@GET, 'GetAuditPersonProfileDetails',1,GETDATE(),1,0)
...
--'/api/audit-person-profile/{peopleIndex}/{historyType}' ends
-------------------------------------------------------
...
BEGIN
    ...
    VALUES (N'20210326064411_personNewAuditApiPermission', N'3.1.4');
END;
