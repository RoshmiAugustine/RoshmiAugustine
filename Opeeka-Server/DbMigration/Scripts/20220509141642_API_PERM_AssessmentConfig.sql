IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20220509141642_API_PERM_AssessmentConfig')
BEGIN
...
--'api/assessment-configurations' starts
IF NOT EXISTS(select ApplicationObjectID from ApplicationObject where [Name] = '/api/assessment-configurations')
   BEGIN
     INSERT INTO ApplicationObject VALUES(3,'/api/assessment-configurations','Get Assessment Configurations',1, 0,GETDATE(),1)
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/assessment-configurations')
   END
ELSE
   BEGIN
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/assessment-configurations')
   END
insert into info.Permission values(@appObjId,@GET, 'Get Assessment Configurations',1,GETDATE(),1,0)
...
--'api/assessment-configurations' ends
-------------------------------------------------------
...
BEGIN
	  ...
	  SET @APPOBJECTID  = (select ApplicationObjectID  from ApplicationObject where NAme = '/api/assessment-configurations');
...
BEGIN
    ...
    VALUES (N'20220509141642_API_PERM_AssessmentConfig', N'3.1.4');
END;
