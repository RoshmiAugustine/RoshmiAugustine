IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20201103060443_API_PERM_Past_Notification_Permission')
BEGIN
...
--'/api/past-notification-list/{personIndex}' starts
IF NOT EXISTS(select ApplicationObjectID from ApplicationObject where [Name] = '/api/past-notification-list/{personIndex}')
   BEGIN
     INSERT INTO ApplicationObject VALUES(3,'/api/past-notification-list/{personIndex}','Get Past Notification List',1, 0,GETDATE(),1)
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/past-notification-list/{personIndex}')
   END
ELSE
   BEGIN
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/past-notification-list/{personIndex}')
   END
insert into info.Permission values(@appObjId,@POST, 'Get Past Notification List',1,GETDATE(),1,0)
...
--'/api/past-notification-list/{personIndex}' ends
-------------------------------------------------------
...
insert into info.RolePermission select ur.UserRoleID  ,  srp.PermissionID from info.SystemRolePermission srp  JOIN UserRole ur ON srp.SystemRoleID = ur.SystemRoleID WHERE srp.PermissionID not in (select PermissionId from info.RolePermission where UserRoleId = ur.UserRoleID)
    ...
    VALUES (N'20201103060443_API_PERM_Past_Notification_Permission', N'3.1.4');
END;
