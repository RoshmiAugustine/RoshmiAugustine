IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20201029192716_API_PERM_ReportPDFDownLoad')
BEGIN
...
--'/api/report/family-report-downloadPDF' starts
IF NOT EXISTS(select ApplicationObjectID from ApplicationObject where [Name] = '/api/report/family-report-downloadPDF')
   BEGIN
     INSERT INTO ApplicationObject VALUES(3,'/api/report/family-report-downloadPDF','CreatePDF',1, 0,GETDATE(),1)
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/report/family-report-downloadPDF')
   END
ELSE
   BEGIN
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/report/family-report-downloadPDF')
   END
insert into info.Permission values(@appObjId,@POST, 'CreatePDF',1,GETDATE(),1,0)
...
--'/api/report/family-report-downloadPDF' ends
-------------------------------------------------------
...
BEGIN
    ...
    VALUES (N'20201029192716_API_PERM_ReportPDFDownLoad', N'3.1.4');
END;
