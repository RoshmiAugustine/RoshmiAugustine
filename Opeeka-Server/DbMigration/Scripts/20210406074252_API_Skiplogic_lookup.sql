IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210406074252_API_Skiplogic_lookup')
BEGIN
...
--'/api/category-item/{questionnaireId}' starts
IF NOT EXISTS(select ApplicationObjectID from ApplicationObject where [Name] = '/api/category-item/{questionnaireId}')
   BEGIN
     INSERT INTO ApplicationObject VALUES(3,'/api/category-item/{questionnaireId}','Get items nd category',1, 0,GETDATE(),1)
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/category-item/{questionnaireId}')
   END
ELSE
   BEGIN
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/category-item/{questionnaireId}')
   END
insert into info.Permission values(@appObjId,@GET, 'Get items nd category',1,GETDATE(),1,0)
...
--'/api/category-item/{questionnaireId}' ends
-------------------------------------------------------
...
--Declaration of variables ends
--'/api/action-type' starts
IF NOT EXISTS(select ApplicationObjectID from ApplicationObject where [Name] = '/api/action-type')
   BEGIN
     INSERT INTO ApplicationObject VALUES(3,'/api/action-type','Get action-type',1, 0,GETDATE(),1)
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/action-type')
   END
ELSE
   BEGIN
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/action-type')
   END
insert into info.Permission values(@appObjId,@GET, 'Get action-type',1,GETDATE(),1,0)
SET @PermissionID = (SELECT PermissionID from info.Permission where ApplicationObjectID = @appObjId AND OperationTypeID = @GET)
insert into info.SystemRolePermission values(@SuperAdmin,@PermissionID);
insert into info.SystemRolePermission values(@OrgAdminRW,@PermissionID);
insert into info.SystemRolePermission values(@OrgAdminRO,@PermissionID);
...
insert into info.RolePermission select ur.UserRoleID  ,  srp.PermissionID from info.SystemRolePermission srp  JOIN UserRole ur ON srp.SystemRoleID = ur.SystemRoleID WHERE srp.PermissionID not in (select PermissionId from info.RolePermission where UserRoleId = ur.UserRoleID)
    ...
    VALUES (N'20210406074252_API_Skiplogic_lookup', N'3.1.4');
END;
