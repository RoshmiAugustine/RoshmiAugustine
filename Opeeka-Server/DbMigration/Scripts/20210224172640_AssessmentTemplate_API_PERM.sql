IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210224172640_AssessmentTemplate_API_PERM')
BEGIN
...
--'/api/assessment-template-json/{questionnaireId}' starts
IF NOT EXISTS(select ApplicationObjectID from ApplicationObject where [Name] = '/api/assessment-template-json/{questionnaireId}')
   BEGIN
     INSERT INTO ApplicationObject VALUES(3,'/api/assessment-template-json/{questionnaireId}','GetAssessmentTemplateFromQuestionnaireId',1, 0,GETDATE(),1)
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/assessment-template-json/{questionnaireId}')
   END
ELSE
   BEGIN
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/assessment-template-json/{questionnaireId}')
   END
insert into info.Permission values(@appObjId,@GET, 'GetAssessmentTemplateFromQuestionnaireId',1,GETDATE(),1,0)
...
--'/api/assessment-template-json/{questionnaireId}' ends
-------------------------------------------------------
...
BEGIN
    ...
    VALUES (N'20210224172640_AssessmentTemplate_API_PERM', N'3.1.4');
END;
