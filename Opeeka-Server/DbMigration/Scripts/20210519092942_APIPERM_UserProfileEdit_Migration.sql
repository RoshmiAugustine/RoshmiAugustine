IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20210519092942_APIPERM_UserProfileEdit_Migration')
BEGIN
...
IF NOT EXISTS(select ApplicationObjectID from ApplicationObject where [Name] = 'api/user-profile')
   BEGIN
     INSERT INTO ApplicationObject VALUES(3,'api/user-profile','UpdateUserProfile',1, 0,GETDATE(),1)
...
ELSE
   BEGIN
     SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = 'api/user-profile')
   END
insert into info.Permission values(@appObjId,@PUT, 'UpdateUserProfile',1,GETDATE(),1,0)
...
BEGIN
    ...
    VALUES (N'20210519092942_APIPERM_UserProfileEdit_Migration', N'3.1.4');
END;
