IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20200811132639_TimePeriodPermissionSettings')
BEGIN
    ...
    --'/api/timeperiod/{daysInEpisode}' starts
    IF NOT EXISTS(select ApplicationObjectID from ApplicationObject where [Name] = '/api/timeperiod/{daysInEpisode}')
       BEGIN
         INSERT INTO ApplicationObject VALUES(3,'/api/timeperiod/{daysInEpisode}','GetTimePeriodDetails.',1, 0,GETDATE(),1)
         SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/timeperiod/{daysInEpisode}')
       END
    ELSE
       BEGIN
         SET @appObjId = (select ApplicationObjectID from ApplicationObject where [Name] = '/api/timeperiod/{daysInEpisode}')
       END
    insert into info.Permission values(@appObjId,@GET, 'GetTimePeriodDetails.',1,GETDATE(),1,0)
    ...
    --'/api/timeperiod/{daysInEpisode}' ends
    -------------------------------------------------------
...
BEGIN
    ...
    VALUES (N'20200811132639_TimePeriodPermissionSettings', N'3.1.4');
END;
