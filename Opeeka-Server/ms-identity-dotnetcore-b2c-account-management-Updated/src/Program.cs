...
namespace b2c_ms_graph
{
    public static class Program
    {
                ...
                while (true)
                {
                    ...
                    switch (decision.ToLower())
                    {
                        ...
                        case "8":
                            ...
                            break;
                        case "9":
                            await UserService.UpdateUserWithCustomAttribute(graphClient, config.B2cExtensionAppClientId, config.TenantId);
                            break;
                        case "10":
                            await UserService.CreateAPIUser_WithCustomAttribute(graphClient, config.B2cExtensionAppClientId, config.TenantId);
                            break;
                        case "11":
                            await UserService.GetUserDetails_FromObjectID(graphClient, config.B2cExtensionAppClientId, config.TenantId);
                            break;
                        case "12":
                            await UserService.BulkUpdateUsersInstanceURL(config, graphClient);
                            break;
                        case "help":
        ...
        private static void PrintCommands()
        {
            ...
            Console.WriteLine("[9]      Update User with custom attributes");
            Console.WriteLine("[10]     Create an API User with custom attributes ");
            Console.WriteLine("[11]     Get all custom attributes for a user by Object ID ");
            Console.WriteLine("[12]     Bulk Update instance URL for users (From json file)");
