IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20200928072136_OTPExiryChangeInEmailTemplate')
BEGIN 

    UPDATE info.Configuration SET VAlue = 'https://www.pcis-staging.com/email-assessment' WHERE ConfigurationParameterContextID 
        in (SELECT ConfigurationParameterContextID FROM info.ConfigurationParameterContext 
        WHERE ConfigurationParameterID 
        in (SELECT ConfigurationParameterID FROM info.ConfigurationParameter WHERE NAME = 'EmailAssessmentURL_Staging'))
...
BEGIN 
    ...
    VALUES (N'20200928072136_OTPExiryChangeInEmailTemplate', N'3.1.4');
END;
